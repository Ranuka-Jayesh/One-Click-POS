...
async function updateCategoryItemCount(categoryId, increment = 1) {
    try {
...
router.post('/', async (req, res) => {
    try {
        ...
        // Log activity
        await logAdminActivity(db, 'Menu Item', 'Added', getAdminUsername(req), `New menu item created: ${name.trim()} (Price: Rs. ${priceValue.toFixed(2)})`, 'success');
        const createdItem = {
            ...newItem,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const result = await menuItemsCollection.updateOne({ _id: menuItem._id }, { $set: updateData });
        ...
        // Log activity
        await logAdminActivity(db, 'Menu Item', 'Updated', getAdminUsername(req), `Menu item updated: ${name.trim()} (Price: Rs. ${priceValue.toFixed(2)})`, 'success');
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await updateCategoryItemCount(menuItem.category, -1);
        // Log activity
        await logAdminActivity(db, 'Menu Item', 'Deleted', getAdminUsername(req), `Menu item deleted: ${menuItem.name}`, 'success');
...
router.patch('/:id/availability', async (req, res) => {
    try {
        ...
        const result = await menuItemsCollection.updateOne({ _id: menuItem._id }, {
            $set: {
                available,
                updatedAt: new Date()
            }
        ...
        // Log activity
        await logAdminActivity(db, 'Menu Item', available ? 'Made Available' : 'Made Unavailable', getAdminUsername(req), `Menu item ${available ? 'made available' : 'made unavailable'}: ${menuItem.name}`, 'success');
