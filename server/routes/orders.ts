...
function getUsername(req: express.Request): string {
  const username = req.headers['x-admin-username'] as string || 
                   req.headers['x-cashier-username'] as string;
  if (username) {
    return username;
  }
...
router.post('/', async (req, res) => {
  try {
    ...
    const newOrder = {
      ...
      items: items.map((item: { menuItem: { id?: string; _id?: string; name: string; description: string; price: number; category: string; image: string }; quantity: number }) => ({
        menuItem: {
    ...
    await logAdminActivity(
      ...
      'Created',
      getUsername(req),
      `New ${orderType} order created: ${newOrder.customerName} (Total: Rs. ${total.toFixed(2)})`,
      'success'
    );
...
router.patch('/:id/status', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Status Updated',
      getUsername(req),
      `Order ${order.orderId} status changed to: ${status}`,
      'success'
    );
...
router.patch('/:id/payment', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Payment Completed',
      getUsername(req),
      `Order ${order.orderId} marked as paid (${paymentMethod})`,
      'success'
    );
...
router.patch('/:id/settle', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Order Settled',
      getUsername(req),
      `Order ${order.orderId} settled by cashier`,
      'success'
    );
...
router.patch('/:id/refund', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Refunded',
      getUsername(req),
      `Order ${order.orderId} marked as refunded`,
      'success'
    );
