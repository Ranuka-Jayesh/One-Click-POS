...
router.post('/cashier/login', async (req, res) => {
  try {
    ...
    if (!cashier) {
      ...
      await logCashierActivity(
        db,
        'Login',
        trimmedUsername,
        'Failed cashier login attempt - User not found',
        'failed',
        typeof ip === 'string' ? ip : 'N/A',
        userAgent
      ).catch(err => console.error('Error logging cashier activity:', err));
    ...
    if (!cashier.isActive) {
      ...
      await logCashierActivity(
        ...
        'Login',
        trimmedUsername,
        'Failed cashier login attempt - Account is deactivated',
        'failed',
        typeof ip === 'string' ? ip : 'N/A',
        userAgent
      ).catch(err => console.error('Error logging cashier activity:', err));
    ...
    if (!isPasswordValid) {
      ...
      await logCashierActivity(
        ...
        'Login',
        trimmedUsername,
        'Failed cashier login attempt - Invalid password',
        'failed',
        typeof ip === 'string' ? ip : 'N/A',
        userAgent
      ).catch(err => console.error('Error logging cashier activity:', err));
    ...
    await cashiersCollection.updateOne(
      { _id: cashier._id },
      { 
        $set: { 
          lastLogin: new Date(),
  ...
  try {
    ...
    if (!user) {
      ...
      await logAdminActivity(
        ...
        'Profile Update',
        username.trim(),
        'Failed profile update - User not found',
        'failed'
      );
  ...
  try {
    ...
    if (!user) {
      ...
      await logAdminActivity(
        ...
        'Password Change',
        username.trim(),
        'Failed password change - User not found',
        'failed'
      );
...
router.get('/cashier/logs', async (req, res) => {
  try {
    ...
    // Build query
    interface CashierLogQuery {
      username?: string;
    }

    const query: CashierLogQuery = {};
    if (username) {
...
router.post('/cashier/change-password', async (req, res) => {
  try {
    ...
    // Update password in database
    const result = await cashiersCollection.updateOne(
      { username: username.trim() },
      { 
        $set: { 
          password: hashedPassword,
...
router.get('/cashier/profile', async (req, res) => {
  try {
    ...
    const cashier = await cashiersCollection.findOne({ username: (username as string).trim() });
...
router.get('/cashier/printers', async (req, res) => {
  try {
    ...
    const printers = await printersCollection
      .find({ cashierUsername: (username as string).trim() })
      .sort({ createdAt: -1 })
      .toArray();
...
router.post('/cashier/printers', async (req, res) => {
  try {
    ...
    const printersToInsert = printers.map((printer: PrinterConfig) => ({
      cashierId: cashier._id.toString(),
...
router.post('/cashiers', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Added',
      fullName.trim(),
      `New cashier created: ${username.trim()}`,
      'success'
    );
...
router.put('/cashiers/:id', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Updated',
      fullName.trim(),
      `Cashier updated: ${cashier.username}`,
      'success'
    );
...
router.delete('/cashiers/:id', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Deleted',
      cashier.fullName,
      `Cashier deleted: ${cashier.username}`,
      'success'
    );
...
router.patch('/cashiers/:id/status', async (req, res) => {
  try {
    ...
    let result;
    try {
      result = await cashiersCollection.updateOne(
        { _id: new ObjectId(id) },
        { 
          $set: { 
            isActive,
    ...
    try {
      await logAdminActivity(
        ...
        `Cashier ${isActive ? 'activated' : 'deactivated'}: ${cashier.username || id}`,
        'success'
      );
    } catch (logError) {
...
router.post('/cashier/cash-out', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Cash Out',
      cashierUsername.trim(),
      `Cash out: Rs. ${amount.toFixed(2)} (Difference: Rs. ${difference.toFixed(2)})`,
      'success'
    );
...
router.post('/tables', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      `New table created: ${label.trim()} (Capacity: ${capacity})`,
      'success'
    );
...
router.put('/tables/:id', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Updated',
      getAdminUsername(req),
      `Table updated: ${label.trim()} (Capacity: ${capacity})`,
      'success'
    );
...
router.patch('/tables/:id/availability', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      getAdminUsername(req),
      `Table ${available ? 'made available' : 'made unavailable'}: ${table.label}`,
      'success'
    );
...
router.post('/categories', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Added',
      getAdminUsername(req),
      `New category created: ${name.trim()} (Icon: ${icon.trim()})`,
      'success'
    );
...
router.put('/categories/:id', async (req, res) => {
  try {
    ...
    await logAdminActivity(
      ...
      'Updated',
      getAdminUsername(req),
      `Category updated: ${name.trim()} (Icon: ${icon.trim()})`,
      'success'
    );
...
router.get('/overview', async (req, res) => {
  try {
    // Define types for menu items and cart items (used throughout this function)
    ...
    // Transform orders to include proper date objects
    ...
    const ordersWithDates = allOrders.map((order) => {
      const orderData = order as OrderFromDB;
      return {
        ...orderData,
    ...
    const itemCounts = new Map<string, { item: MenuItemData, quantity: number, revenue: number }>();
    last7DaysOrders.forEach((order) => {
      if (order.items && Array.isArray(order.items)) {
        order.items.forEach((cartItem) => {
          const cartItemData = cartItem as CartItemData;
    ...
    const dayCounts = new Map<number, { day: string, orders: number, revenue: number }>();
    ...
    // Peak hours (all time)
    const hourCounts = new Map<number, { hour: number, orders: number, revenue: number }>();
    ...
    const menuItemsMap = new Map<string, MenuItemData>();
    allMenuItems.forEach((item) => {
      const itemData = item as MenuItemFromDB;
